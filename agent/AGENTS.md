...
`read` is a native pi tool (not bash, not `cat`) for reading files you intend to edit.
...
Wiki repos live at `~/repos/llm-wiki/` (or any directory with an `AGENTS.md` that
references this pattern). To start wiki work in any session:

```
/skill:wiki-ops     — load ingest / query / lint workflows
```

The skill source of truth is `~/repos/llm-wiki/.pi/skills/wiki-ops/SKILL.md`
(globally symlinked from `~/.pi/agent/skills/wiki-ops`). Edit the skill there;
the symlink keeps both paths current.
