...
remove_harness() {
  ...
  case "$harness" in
    pi)
      ...
      unlink "$HOME_DIR/repos/agents/pi"         2>/dev/null || true
      ;;
    claude-code)
    ...
    copilot)
      ...
      unlink "$HOME_DIR/repos/agents/copilot-cli"        2>/dev/null || true
      ;;
    *)
...
if [ -d "$HOME_DIR/.pi/agent" ]; then
  ...
  link "$REPO/harnesses/pi/agents"             "$HOME_DIR/repos/agents/pi"
  wire_skill "wiki-ops"
else
...
link "$REPO/harnesses/copilot/agents"          "$HOME_DIR/repos/agents/copilot-cli"
